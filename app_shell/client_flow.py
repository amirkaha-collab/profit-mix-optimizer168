...
def _step_data_intake(case, df_long, engine: WorkflowEngine) -> None:
    """
    Step 2: Data intake.
    Real clearing-report file uploader + manual add + edit.
    Reuses existing parse_clearing_report engine — no duplication.
    """
    n = len(case.all_holdings)

    # Status banner
    if n == 0:
        st.info("העלה דוח מסלקה פנסיונית (XLSX) או הוסף מוצרים ידנית.", icon="📥")
    else:
        n_miss = sum(1 for h in case.all_holdings
                     if not h.get("equity_pct") and h.get("equity_pct") != 0.0)
        pct  = int((n - n_miss) / max(n, 1) * 100)
        clr  = "#065f46" if pct == 100 else ("#92400e" if pct < 70 else "#1d4ed8")
        bg   = "#d1fae5" if pct == 100 else ("#fef3c7" if pct < 70 else "#eff6ff")
        st.markdown(
            f'<div style="background:{bg};border-radius:10px;padding:9px 14px;'
            f'direction:rtl;text-align:right;margin-bottom:10px">'
            f'<strong style="color:{clr}">{n} מוצרים | {n-n_miss} עם חשיפות | שלמות: {pct}%</strong></div>',
            unsafe_allow_html=True,
        )

    # ── Real file uploader ─────────────────────────────────────────────────
    st.markdown("#### 📤 ייבוא דוח מסלקה")
    uploaded = st.file_uploader(
        "קובץ XLSX של דוח מסלקה פנסיונית (גמל-נט / משרד האוצר)",
        type=["xlsx", "xls"],
            ...
            try:
                ...
                if err_msg and not result:
                    ...
                    if holdings_raw:
                        ...
                        # Compute baseline immediately
                        bl = _compute_baseline_from_holdings(holdings_raw, df_long)
                        if bl:
                            from case_management import PortfolioSnapshot
                            _upd.current_snapshot = PortfolioSnapshot.from_baseline_dict(bl, _upd.current_total)
                            st.session_state["portfolio_baseline"] = bl
                            _upd.step_done[STEP_SNAPSHOT] = True
                        CaseStore.save(_upd)
                        st.success(f"נטענו {len(holdings_raw)} מוצרים מדוח המסלקה.")
                        st.rerun()
                    else:
    ...
    try:
        ...
        st.warning(f"טופס הוספה: {e}")

    # ── Edit/remove existing ───────────────────────────────────────────────
    case = CaseStore.get()
    if case.all_holdings:
        with st.expander(f"✏️ ערוך / הסר ({len(case.all_holdings)} מוצרים)", expanded=False):
            try:
    ...
    if case.all_holdings:
        ...
        _next_cta(
            "נתונים נקלטו",
            f"{len(case.all_holdings)} מוצרים בתיק — ניתן להמשיך.",
            "📊 המשך לשלב 3 — תמונת מצב",
            "intake_next_v2",
            STEP_SNAPSHOT,
        )
    else:
        st.info("הוסף לפחות מוצר אחד להמשך.", icon="ℹ️")
